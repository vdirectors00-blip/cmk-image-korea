...
> **보안 baseline (v1.2 확정)**: 모든 폼은 다음 3개 보안 필드를 공통으로 포함한다.
...
> - `submittedAt` (클라이언트) — 폼 로드 시각 vs 제출 시각 차이로 너무 빠른 제출(<3초) 차단.
>
...
#### Turnstile widget 사용 예 (Astro/React)
```tsx
// 프론트엔드 폼 컴포넌트 안
...
// onTurnstileSuccess(token) 안에서 hidden input의 value에 토큰 주입
```
...
const RATE_LIMIT_MAX  = 5;             // 1시간 내 같은 IP 5회
...
**이 문서는 v1.2이며, 개발 진행 중 발견되는 결정 사항은 본 문서에 추가·갱신함.**
