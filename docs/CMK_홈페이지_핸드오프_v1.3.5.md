...
<script is:inline data-filter-script="category">
  (function () {
    const KEY = "category";
    const root = document.querySelector("[data-filter-root]");
    const cards = document.querySelectorAll("[data-card]");
    const navLinks = document.querySelectorAll(`[data-filter-nav="${KEY}"] a`);

    function apply() {
      const url = new URL(window.location.href);
      const value = url.searchParams.get(KEY);
      cards.forEach((c) => {
        const match = !value || c.dataset.category === value;
        c.classList.toggle("hidden", !match);
      });
      navLinks.forEach((a) => {
        const target = new URL(a.href, window.location.origin);
        const isActive = (target.searchParams.get(KEY) ?? "") === (value ?? "");
...
- `/contact` — `form` (personal/corporate/academy)

> 위 4 곳은 같은 단일 클라이언트 JS 패턴이라 함수화 가능하지만, 현재는 페이지 인라인이 디버깅에 유리. 향후 정리 가치 있음.
...
- [ ] 로컬에서 폼 제출 → Sanity 저장 → 이메일 발송 → Resend 의도적 실패 시 silent log + Sanity 저장은 정상 동작 end-to-end 확인
...
**이 문서는 v1.3.5이며, 개발 진행 중 발견되는 결정 사항은 본 문서에 추가·갱신함.**
