...
페이지마다 고유 컴포넌트로 빌드하므로, `page` 타입은 **간단한 텍스트 페이지 (개인정보처리방침·이용약관 등)에만** 사용:
```ts
{
  name: "page",
  type: "document",
  fields: [
    ...
    { name: "slug",  type: "slug", options: { source: (doc: any) => doc?.title?.ko ?? "" } },
    { name: "effectiveDate", type: "date" },    // optional — 약관 시행일 (YYYY-MM-DD)
    { name: "version",       type: "string" },  // optional — 개정 버전 표기 (예: "1.0")
    { name: "body",  type: "localeBlockContent" },
    { name: "seo",   type: "seo" },
  ],
}
...
> **slug 등록 규칙**: `/privacy` 페이지는 slug `privacy`, `/terms` 페이지는 slug `terms` 로 고정. 사이트가 slug 로 문서를 조회하므로 이 값 변경 시 URL 깨짐.
> **siteSettings 에는 약관 필드 없음** (v1.3.10에서 제거) — 약관 본문은 반드시 page 타입으로 관리.
> **Studio 등록 상세 가이드**: `docs/legal/Studio_약관등록_가이드.md` 참조.
> ⚠️ `body.ko` 는 `rule.required()` 제약 — 빈 본문으로 저장 불가. 법무 검토 전이라면 `(준비 중)` 1줄이라도 입력 후 게시.
...
#### 1차 오픈 시 동작 (v1.3.12 갱신)
- **인덱스 페이지는 항상 노출** — 3 영역 카드 (퍼스널컬러 / 스타일 / 골격·헤어) 한 줄 카피로 노출 ([[legacy-content-audit]] §4 결정 3)
- Sanity `labArticle` 컬렉션 비어있으면 인덱스 페이지 하단의 "최신 글" 그리드는 placeholder ("Articles coming soon")
- 컬렉션에 1개 이상 들어오면 자동으로 카드 그리드 활성화
- Header `navigation` 에서 메뉴 항상 노출 (조건부 숨김 제거)

#### 페이지 구조
- 섹션 1 — 헤더 "Image Lab" + 부제 "Research & Notes"
- 섹션 2 — **3 영역 카드** (퍼스널컬러 · 스타일 · 골격·헤어) — 각 한 줄 카피
  - 퍼스널 컬러 — 자연적인 색채조화로 사람에게 어울리는 색을 진단하는 프로그램
  - 스타일 — 바디 타입 진단을 통해 자신만의 어울리는 소재·실루엣·패턴을 알 수 있는 시스템
  - 골격·헤어 — 얼굴형과 골격에 따라 헤어 스타일과 헤어 웨어를 연출하는 프로그램
- 섹션 3 — 분야 필터 탭 (퍼스널컬러 / 스타일 / 골격 / 헤어 / 전체) — labArticle 등록 후 활성화
- 섹션 4 — 카드 그리드 (Marie Claire 스타일) — labArticle 등록 후 노출
- 무한 스크롤 또는 페이지네이션
...
- `/privacy` — 개인정보처리방침 (Sanity `page` 타입, slug: `"privacy"` 로 조회)
- `/terms` — 이용약관 (Sanity `page` 타입, slug: `"terms"` 로 조회)

PageHead 세리프 italic lowercase + 시행일·버전 메타 + PortableText 본문 렌더링. 본문 미등록 시 "준비 중" placeholder 자동 노출 (페이지 구조는 항상 정상). §4.3.10 참조.
...
```
[LOGO]                  [About  Consulting  Academy  Image Lab  Community  Contact]                  [    ]
좌측                                          정중앙 (absolute centering)                           우측 (현재 비움)
```

- 좌측: CMK 로고 (텍스트 또는 SVG, `font-serif text-2xl`)
- **정중앙**: nav 메뉴 `About / Consulting / Academy / Image Lab / Community / Contact` (v1.3.13 — Image Lab 정식 추가, 6개 메뉴). 1차 오픈 KO only 라 언어 토글 없음. Image Lab 위치는 Academy 다음 (콘텐츠 카테고리 그룹 — Image Lab · Community)
- v1.3.13 — Footer Site Map 도 동일하게 6개로 동기화 (Image Lab 추가)
...
- 컬럼 2: Site Map (About · Consulting · Academy · Image Lab · Community · Contact — 6 메뉴, Header 와 동기화. v1.3.13 → v1.3.14)
...
### Day 9 — Phase 6 사전 준비 + 약관 페이지 골격 (v1.3.10 업데이트)
- [x] **약관 페이지 골격 구현 완료** (v1.3.10):
  - `/privacy.astro` + `/terms.astro` 신설 — PageHead 매거진 톤 + 시행일·버전 메타 + PortableText 본문 렌더링
  - `PortableText.astro` 공용 컴포넌트 신설 (`apps/web/src/components/common/`) — 외부 패키지 없이 h2/h3/p/strong/em/link/blockquote 지원
  - `.prose-legal` CSS 추가 (`global.css`) — 법조문 읽기용 타이포그래피
  - Footer 의 "준비 중" `aria-disabled` → 정식 `<a href>` 링크 전환
  - 약관 초안 2건 (`docs/legal/`) + Studio 등록 가이드 (`docs/legal/Studio_약관등록_가이드.md`)
- [ ] **약관 콘텐츠 입수 푸쉬**: 클라이언트한테 초안 (`docs/legal/`) 전달 + 법무 검토 요청. 검토 완료 본문을 Studio 에 등록하면 사이트 자동 노출.
...
- ✅ **기존 URL → 새 URL 301 리다이렉트 매핑** (v1.3.12 신설) — 기존 cmkimage.co.kr 의 22 페이지 + 게시판 85건 ≈ 100+ 인덱싱된 URL 의 SEO 누적치 유지 + 외부 인용 링크 보호. Cafe24 정적 호스팅이므로 `.htaccess` 또는 `_redirects` 패턴 활용. 매핑표 작성은 lead time 큼 → Day 9~10 단계에서 작업 시작 권장. 매핑 작업: `[[legacy-content-audit]] §12 URL 리다이렉트 매핑` 참조
- ✅ 클라이언트 검수 완료
...
#### 13.5.6 컨텍스트 거대화 시 JSON 직렬화 깨짐 — UTF-16 surrogate pair (v1.3.13)

Claude Code 세션의 컨텍스트가 누적되면 도구 응답 직렬화 단계에서 UTF-16 surrogate pair (이모지·일부 한자·옛한글 같은 BMP 외 문자) 가 잘린 채 전송되어 다음 API 에러로 클로드A·B 모두 멈출 수 있다:

```
API Error: 400 The request body is not valid JSON: no low surrogate in
string: line 1 column 160097 (char 160096)
```

**실 사고 (Day 10 진입 직전 — 2026-05-25)**:
클로드A 가 Day 10 디자인 검수 사전 작업 (페이지별 코드 스캔 + §10 미결 11건 매핑표) 누적 후 사용자 결정 항목 추출 단계에서 위 에러 반복 발생. `/model` 변경·재시도 모두 무효. 컨텍스트에 박힌 직렬화 깨짐은 영구.

**복구 절차** (우선순위 순):
1. **산출물 보존이 우선** — 사용자가 마지막 정상 출력 (검수표·매핑·결정 회신 등) 을 손으로 복사해 `docs/_wip/<날짜>_<주제>.md` 임시 파일로 저장
2. `/clear` 로 세션 초기화
3. 새 세션에서 `_wip/` 파일 + 메모리 + 핸드오프 읽고 작업 이어서
4. 보존 단계가 어렵거나 산출물 재실행 비용이 낮으면 바로 `/clear` (속도 우선)
5. 절대 금지: 같은 명령 무한 재시도 (에러 그대로 반복) / 작업 산출물을 다시 토하게 시도 (도구 응답이 또 깨짐)

**예방**:
- 한 세션의 누적 도구 응답이 너무 커지면 (수만 자 페이지 코드 스캔 등) 중간에 산출물을 `docs/` 또는 `docs/_wip/` 에 파일로 저장 후 다음 단계 진입. transcript 안에만 두지 말 것
- 큰 매핑·검수표는 한 응답에서 다 토하지 말고 카테고리별로 분할 출력
...
**이 문서는 v1.3.14이며, 개발 진행 중 발견되는 결정 사항은 본 문서에 추가·갱신함.**

**Changelog**
- v1.3.14 — Day 10 디자인 검수 라운드 결과 + 사용자 디자인 개선 18건 + 콘텐츠 시드 + 클로드B v1.3.13 검수 후보 4건 모두 패치:
  - **`apps/studio/scripts/seed.ts` 신설** — audit §11 입력 순서 자동화. siteSettings(1) + person CEO(1) + stat(3) + service(4) + academyCourse(4) + post(4) + clientLogo(10 임시) + page(2 placeholder) = **29 문서 createOrReplace**. 토큰은 `$env:SANITY_AUTH_TOKEN` 일회성 주입 (`Read-Host -AsSecureString` 패턴). 실행 = `npx sanity exec scripts/seed.ts`. `apps/studio/package.json` 에 `@sanity/client` devDep 추가 (pnpm transitive 차단 우회)
  - **클라이언트 로고 (LogoWall) 임시 시드** — audit §2-4 history 10건 (문체부·국무총리실·감사원·합참·국방부·삼성·SK·현대차·동아일보·이화여대). `consentGiven=true` 로 시드해 LogoWall 노출. 정치인·외국·동의 어려운 항목 제외. 정식 명단·로고·동의는 §9-2 #7 클라이언트 컨펌 후 Studio 에서 교체
  - **HOME Hero 사이즈** — `h-[100svh]` → `h-[calc(100svh-89px)]`. Header 높이 (`py-8`*2 + 로고 24px + border 1px = 89px) 포함해 화면에 꽉 차게. 사용자 시각 확인 OK
  - **HOME Stats grid 일치** — `StatsCounter.tsx` `slice(0, 4)` → `slice(0, 3)`. audit §11 #4 stat 3건 의도 + `md:grid-cols-3` 양 끝 일치. 클로드B v1.3.13 후보 #1 해소
  - **HOME Stats 12-col 재구성** — 좌 7 (eyebrow `Heritage` + 큰 세리프 `수치로 본 CMK.` + 매거진 톤 카피 + StatsCounter 3건) / 우 5 (`aspect-[4/5]` 사진 placeholder, 클라이언트 사진 제공 후 교체). 단순 정적 표시 → 매거진 톤 섹션화
  - **HOME StatsCounter 폰트 축소 + `whitespace-nowrap`** — clamp(48,7vw,96) → clamp(24,3vw,48). col-span-7 안 3 cols 에서 "12,000명" 두 줄 깨짐 방지
  - **HOME ContactCta 동등 폭 + 화살표 제거** — `flex flex-wrap` → `grid grid-cols-3 max-w-2xl` + 셋 다 `variant="secondary" + "w-full justify-center"`. "기업 출강 문의 →" 화살표 제거. Corporate 만 튀던 시각 강조 해소
  - **HOME ServiceCards hover** — `origin-bottom-left transition-transform duration-500 hover:scale-[1.02]`. 카드 hover 시 우상단 방향 살짝 커짐. 기존 이미지 group-hover:scale-[1.03] 와 누적
  - **Button class prop 추가** — `<Button class="w-full justify-center" ...>` 같이 외부 폭·정렬 지정 가능 (ContactCta 동등 폭 위해. 일반 보조 기능)
  - **Header active/hover 패턴 정정** — underline arbitrary value 미작동 → border-b 패턴으로 교체 (`baseLinkClass: "inline-block ... pb-1 border-b ..."`, active = `border-foreground`, 비active = `border-transparent` + hover = `border-foreground hover:text-foreground`). 점프 없음 + jit 안전. 사용자 시각 OK
  - **Contact / Consulting 별표 ★ 제거** — `ContactPageHead.astro` nav 라벨 `Corporate ★` → `Corporate`, `contact.astro` 폼 eyebrow `Corporate Inquiry ★` → `Corporate Inquiry`, `consulting/index.astro` 카드 eyebrow `Corporate ★` → `Corporate`, `consulting/[slug].astro` 카테고리 라벨 `Corporate ★ · 본진` → `Corporate`. 사용자 결정 "Corporate 만 튀게 안 되게 + 사이트 전체 일관성"
  - **About `Founder` → `CEO` 라벨 통일** (v1.3.13 결정 #3 코드 적용분 명문화) — `AboutPageHead.astro:13` "Founder" → "CEO", `CeoProfile.astro:50` "Founder · CEO" → "CEO", `CeoIntro.astro:42` "Founder" → "CEO"
  - **About CEO 사진 fallback** — `apps/web/public/images/ceo-fallback.jpg` 신설 (스크린샷 사진 복사). `CeoProfile.astro` + `CeoIntro.astro` 의 `ceo.image` 비어있을 때 fallback 이미지 자동 노출 (`object-position: 50% 25%` 머리 위쪽). Sanity Studio 에서 정식 이미지 업로드 시 자동 교체
  - **About CEO Credentials 임시 숨김** — `CeoProfile.astro:80-98` Credentials 섹션 + placeholder 박스 통째 제거. Sanity 의 ceo.credentials 7건은 시드 그대로 보존. 클라이언트 정식 약력 제공 시 코드만 풀어주면 됨
  - **About Philosophy = audit §2-1 정식 카피 + zigzag** (대형 변경) — 임의 placeholder (Image / Identity / Influence) 폐기. **CMK 3C 정신** (Creative · People · Communication — Creative Partner / Expert Group / Service) + **3축 방법론** (Appearance · Behavior · Communication — 호감 · 기억 · 감동) 정식 적용. 디자인 = 큰 fade 숫자 (3C) / 큰 fade 한글 (3축) + 좌우 zigzag (이미지 박스 폐기, 페이지 길이 압축). audit §2-1 운율 살아있음 (호감 → 기억 → 감동) 그대로 활용. 출처 = 기존 cmk 홈페이지 SERVICE CONTENTS 페이지 본인 카피. §13.3 #2 placeholder 항목 해소
  - **About HistoryTimeline = audit §2-4 + zigzag 매거진 패턴 + 2008-2016 만** — 임의 1995/2005/2010/2018/2024 placeholder 폐기. audit §2-4 history 9 마일스톤 (2008 설립 / 2009 문체부 / 2010 총리실 / 2011 감사원·이화여대 / 2012 합참 / 2013 감사원·삼성·SK / 2014 현대차·새누리당 / 2015 현대차 제네시스 / 2016 동아일보·GIRL UP). 사용자 결정 정정 — 2017 이후는 일단 미표시 (클라이언트 자료 확정 후 추가). 디자인 = 가운데 vertical line + 좌우 zigzag + 큰 italic 연도 (clamp 40-84px). 좌측 카피 "현장 20여 년의 결을 따라." + audit §2-4 도입 카피
  - **About PeopleGrid 디자인 정정 + placeholder 자리잡기** — grid cols `2 md:3 lg:4` → `3 md:4 lg:6` (카드 폭 절반). title·role 표시 제거 (이름만). 데이터 0건일 때 placeholder 카드 12개 grid 자동 노출 (시각 자리잡기용)
  - **HOME ColorQuizPlaceholder → ColorQuizSection 교체** (v1.3.13 결정 #2 적용분 명문화) — `ColorQuizSection.astro` 신설로 매거진 톤 헤더 유지 + `<ColorQuiz client:visible />` React island embed. ColorQuizPlaceholder.astro 는 미사용 상태로 보존
  - **Image Lab 인덱스 페이지 + 메뉴 신설** (v1.3.13 결정 #1 적용분 명문화) — `apps/web/src/pages/image-lab/index.astro` + `[slug].astro` 신설, `components/lab/LabPageHead.astro` + `LabAreaCards.astro` 신설. Header / Footer Site Map 도 6 메뉴 (Image Lab 정식 추가, Academy ↔ Community 사이)
  - **§6.5 Image Lab 명세** — `LabPageHead` 가 분야 필터 (All/Color/Style/Body/Hair) — `labArticle` 0건이면 nav 숨김. `LabAreaCards` = §6.5 섹션 2 한 줄 카피 3 영역 카드. `image-lab/[slug].astro` 상세 페이지 (cover + 헤더 + PortableText + 본문 미등록 시 placeholder)
  - **consulting/[slug] PortableText body 렌더링 활성화** — 시드된 service.body (3 H2 + 본문 6단락) 가 코드에 미반영 → "Phase 3 골격 · placeholder" 그대로 표시되던 실 버그. PortableText 추가 + Tailwind arbitrary variant 매거진 톤 (`[&_h2]:font-serif [&_h2]:text-3xl [&_h2]:mt-14 ... [&_p]:leading-[1.85] [&_p]:text-soft [&_blockquote]:...`). corporate/executive/public/media 4 페이지 모두 풍부해짐
  - **클로드B v1.3.13 검수 후보 #2 (핸드오프 §7.2 Footer 4메뉴)** 해소 — line 1220 의 `빠른 링크 (About, Consulting, Academy, Contact)` → `Site Map (About · Consulting · Academy · Image Lab · Community · Contact — 6 메뉴, Header 와 동기화)`
  - **클로드B v1.3.13 검수 후보 #3 (audit §10 본문)** 해소 — `docs/legacy-content-audit.md` §10 11 항목 체크박스 갱신 (해소 ✓ 4건 / 부분 해소 ◐ 3건 / 미해소 ☐ 4건). 각 항목 별로 v1.3.13/v1.3.14 결정 인용. §10 상단에 v1.3.14 진행 상태 박스 추가
  - **클로드B v1.3.13 검수 후보 #4 (image.ts:2 sub-path import)** 해소 — `apps/web/src/lib/image.ts:2` `import type { SanityImageSource } from "@sanity/image-url/lib/types/types"` 제거. `Parameters<typeof builder.image>[0]` 패턴으로 외부 dep 없이 타입 추출. tsc --noEmit 에러 0건 기대
  - **사고 회고** — Day 10 진입 시 클로드A 가 UTF-16 surrogate pair 직렬화 깨짐 사고 (§13.5.6) → 이전 클로드B 가 클로드A 역할 인계 → 시각 검수 + 코드 작업 + 시드 진행. 메모리 [[feedback-collaboration-pattern]] 사이클 그대로 작동
  - **v1.3.15+ 후속 작업 명문화 (보류)** — 강사진 73명 axes 스키마 + 개별 페이지 + 사진 업로드 (사용자가 스크린샷 OCR 로 이름 73건 확보 완료, Appearance 56 / Behavior 7 / Communication 10. 동명이인 이지연 2명·CEO 조미경 겸직 확인). 진행 시점 = 사용자가 강사 사진 (개별 파일) + axes 매핑 정보 + 약력 데이터 제공 시. 디자인 = 개별 페이지 (`/about/people/[slug]`, VOGUE contributor 패턴) — 73 페이지 SSG 부담 없음
  - **v1.3.15+ 후속 — consulting 본문 콘텐츠 보강** — 사용자가 스크린샷으로 추가 콘텐츠 모아 시드 body 확장 예정
- v1.3.13 — Day 10 디자인 검수 사전 작업: audit §10 미결 11건 중 사용자 결정 6건 코드 반영 + JSON 직렬화 깨짐 사고 1건 명문화:
  - **Image Lab 인덱스 페이지 + 메뉴 신설** (audit §0 결정 #3 실구현) — `apps/web/src/pages/image-lab/index.astro` + `[slug].astro` 신설, `components/lab/LabPageHead.astro` + `LabAreaCards.astro` 신설. §6.5 명세 그대로: 헤더(image lab) → 3 영역 카드(퍼스널컬러·스타일·골격·헤어 한 줄 카피) → labArticle 분야 필터(labArticle 0건 시 nav 숨김) → Latest Notes 카드 그리드(0건 시 Coming Soon placeholder). 글 상세는 cover + PortableText + 본문 미등록 시 placeholder
  - **Header / Footer Site Map Image Lab 메뉴 추가** — 5개 → 6개. Academy 다음, Community 앞 (콘텐츠 카테고리 그룹). `components/layout/Header.astro` + `Footer.astro` navItems / siteMap 동기화. §7.1 ASCII 다이어그램·서술 갱신
  - **HOME ColorQuiz embed** — `components/home/ColorQuizSection.astro` 신설로 기존 ColorQuizPlaceholder 교체. 매거진 톤 헤더 (eyebrow / 큰 세리프 제목 / 부제) 유지 + `<ColorQuiz client:visible />` React island embed. resultBase 기본값 그대로 `/color-quiz/result/[type]` 로 이동. ColorQuizPlaceholder.astro 는 미사용 상태로 보존 (사용자 결정 후 삭제 결정)
  - **HOME Stats 카운트업 React island** — `components/home/StatsCounter.tsx` 신설 (IntersectionObserver threshold 0.3 진입 시 0 → value, ease-out cubic, 1.6s, `prefers-reduced-motion` 대응 즉시 표시). `Stats.astro` 는 정적 표시 제거하고 island 위임만
  - **About 라벨 Founder → CEO 일관성 회복** (audit·핸드오프 §6.2 명세 동기화) — `AboutPageHead.astro:13` "Founder" → "CEO", `CeoProfile.astro:50` "Founder · CEO" → "CEO", `CeoIntro.astro:42` "Founder" → "CEO". 코드 레벨 `ceo` 식별자(변수·URL slug·Sanity 필드) 는 유지 — 사용자 노출 라벨만 통일
  - **HistoryTimeline 2016 + placeholder** (audit §0 결정 #4 실구현) — 임의 placeholder 였던 `2018: 방송 인터뷰·강연` / `2024: Image Lab 콘텐츠 아카이브 시작` 마일스톤 2건 제거. 정식 항목 1995/2005/2010 유지 + 2016 마일스톤("이미지컨설팅 교육과정 정착") 추가 + 별도 placeholder 행 ("2017 — present · 최근 활동은 클라이언트 자료 입수 후 업데이트 예정", 빈 마커 + faint 색). 좌측 캡션도 "Recent activities to be added" (영문 italic) 강화. 정확한 마일스톤은 §9-2 클라이언트 컨펌 패키지 회신 후 갱신
  - **HOME 시그니처 카드 4장 유지 결정 명문화** (audit §10 항목 2 해소) — Image Lab 인덱스 신설로 콘텐츠 진입 경로 확보됨에 따라 HOME 시그니처 카드는 핵심 4 서비스 (Corporate / Executive / Public / Media) 에 집중. ServiceCards.astro `slice(0, 4)` 그대로. 시그니처 카드 5장 확장 옵션은 폐기
  - **§13.5.6 신설** — Claude Code 컨텍스트 거대화 시 UTF-16 surrogate pair 직렬화 깨짐 사고 패턴 + 복구 절차 (산출물 손복사 → `docs/_wip/` 저장 → `/clear`). 2026-05-25 실 사고 (Day 10 진입 직전, 클로드A 컨텍스트 ~160K 문자 시점에서 발생) 기준 명문화
  - **§7.1 Header 명세 갱신** — ASCII 다이어그램·서술 모두 6 메뉴 (Image Lab 정식 추가) 로 갱신
  - 발견 LOW 1건 (v1.3.14 후보) — `apps/web/src/lib/image.ts:2` 의 `@sanity/image-url/lib/types/types` sub-path import 가 TS module resolution 경고 (`Cannot find module ... or its corresponding type declarations`). 빌드는 통과지만 tsc --noEmit 시 에러로 보임. `import type { SanityImageSource } from "@sanity/image-url"` 의 공식 export 로 교체 권장
  - 보존 원칙 회고 — v1.3.10 / v1.3.11 핸드오프 파일은 별도 보존되지 않고 v1.3.12 Changelog 안에 통합 기록됨. v1.3.13 부터 다시 별도 파일 보존 패턴 복귀 (`docs/CMK_홈페이지_핸드오프_v1.3.13.md`)
...
- v1.3.12 — Day 9 콘텐츠 감사 (`docs/legacy-content-audit.md` 작성) + 클로드B 검수 발견 1·3 반영:
  - **§6.5 IMAGE LAB 명세 갱신** — 기존 "labArticle 비어있으면 메뉴 자체 숨김" → "**인덱스 페이지는 항상 노출** (3 영역 카드 + 한 줄 카피)". labArticle 비어있을 시 카드 그리드만 placeholder. 사용자 결정 [[legacy-content-audit]] §4 결정 3 (B 옵션 — 한 줄 카피 1차 노출)
  - **§13.2 Phase 6 사전 준비 보강** — 기존 사이트 100+ URL 의 **301 리다이렉트 매핑** 작업 추가. Cafe24 정적 호스팅 `.htaccess` 또는 `_redirects` 패턴. 매핑표 작성 lead time 큼 → Day 9~10 단계 시작
  - **§7.1 Header 메뉴 표기** — Image Lab "조건부 — 2차 오픈" → "§6.5 갱신 — 인덱스 항상 노출"
  - **`docs/legacy-content-audit.md` 신설** — 22 페이지 콘텐츠 감사 마스터 문서 (11 섹션 + URL 리다이렉트 §12). 의사결정 4건 확정 (Face Type 보류 / 개인이미지 미추가 / Image Lab 한 줄 카피 / History 2016 까지)
- v1.3.11 — 클로드B v1.3.10 검수 메모 4건 반영:
  - **`PortableText.astro` 리스트 지원 추가** — `renderBlocks()` 함수 도입, 연속 listItem 블록을 `<ul>`/`<ol>`로 그룹핑. 약관·개인정보처리방침 번호 목록 정상 렌더링. `.prose-legal` CSS에 `ul/ol/li` 스타일 추가
  - **§6.9 약관 페이지 명세 갱신** — `siteSettings.privacyPolicy/termsOfUse` → `page` 타입 (slug: "privacy"/"terms") 으로 교체. siteSettings 약관 필드는 v1.3.10에서 제거됨
  - **`formatDate` 타임존 버그 수정** — `privacy.astro` + `terms.astro` 에서 `new Date(iso)` (UTC 자정 파싱) → `iso.split("-").map(Number)` 방식으로 변경. 빌드 서버 TZ 무관하게 날짜 정확
  - **미사용 import 제거** — `privacy.astro` + `terms.astro` 에서 `pickLocale` import 제거
- v1.3.10 — Day 9 약관 페이지 골격 구현 + 클로드B 검수 발견 1건 반영:
  - **§4.3.10 page 스키마 보강** — `effectiveDate` (date, optional) + `version` (string, optional) 필드 추가. 약관 시행일·개정 버전 명시. 다른 page 문서에는 영향 없음 (optional)
  - **siteSettings 스키마 정리** — 기존 `privacyPolicy` + `termsOfUse` localeBlockContent 필드 제거 (클로드B 발견 1 반영). 약관 본문 경로를 `page` 타입으로 단일화. Studio 혼선 방지
  - **`PortableText.astro` 공용 렌더러 신설** (`apps/web/src/components/common/`) — 외부 패키지 없음, h2/h3/p/strong/em/link/blockquote 지원. 약관·법적 텍스트 전용 `.prose-legal` CSS 추가 (`global.css`)
  - **`/privacy.astro` + `/terms.astro` 신설** — PageHead 세리프 italic lowercase ("privacy"·"terms") + 시행일·버전 메타 + PortableText 본문 + placeholder fallback (본문 없으면 "준비 중" 메시지). 콘텐츠와 무관하게 페이지 구조 완성
  - **Footer 법적 링크 정식 전환** — `aria-disabled` span → `<a href="/privacy">`, `<a href="/terms">` 정식 링크
  - **약관 초안 2건 + Studio 등록 가이드** — `docs/legal/개인정보처리방침_초안.md` (PIPA 기준 17개 항목, CMK 실 데이터 흐름 — Sanity/Cloudflare/Resend 위탁사 명시), `docs/legal/이용약관_초안.md` (일반 서비스 약관 11개 항목), `docs/legal/Studio_약관등록_가이드.md` (slug 고정·required 제약·[클라이언트 확인 필요] 목록 포함)
  - **`types/sanity.ts` Page 타입 보강** — effectiveDate·version 추가, SiteSettings 에서 privacyPolicy·termsOfUse 제거
  - **`queries.ts` pageBySlugQuery 동기화** — effectiveDate·version 필드 추가
