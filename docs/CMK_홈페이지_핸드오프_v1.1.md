...
|---|---|---|---|---|
| **GitHub** (저장소) | Phase 1 | V-Directors | V-Directors (유지) | 없음 |
| **Sanity** (CMS) | Phase 2 | V-Directors organization | 클라이언트 organization | "Transfer ownership" 버튼 — 프로젝트 ID·토큰·설정 유지, 사이트 재빌드 불필요 |
| **Cloudflare** (Worker) | Phase 4 | V-Directors | 클라이언트 | 새 계정에서 `wrangler deploy` + secrets 재등록 (약 5분) |
| **Resend** (이메일) | Phase 4 | 발신 도메인 따라 결정 | 클라이언트 | 개발 중엔 V-Directors 도메인 발신, 정식 오픈 시 클라이언트 도메인으로 교체 |
...
- V-Directors Cloudflare 계정에서 Worker 개발·배포 (Worker URL은 `*.workers.dev` 기본값 사용)
- Resend 가입 — 발신 도메인 결정:
...
- ✅ **Cafe24 FTP 정보** (호스트·계정·비번·경로)
- ✅ **cmkimage.co.kr A 레코드 변경 권한** (Cafe24 IP로 변경)
- ✅ **클라이언트 Resend 도메인 DKIM/SPF 인증** (정식 발신 주소로 교체 시)
- ✅ 클라이언트 검수 완료

**오픈 후 (이관 단계):**
- 클라이언트 Sanity organization 생성 → V-Directors organization에서 Transfer ownership 실행
- 클라이언트 Cloudflare 가입 → 같은 Worker 코드를 클라이언트 계정에서 재배포 + secrets 재등록
- 사이트 환경변수 `PUBLIC_WORKER_URL` 만 새 Worker URL로 교체 후 재빌드

> 💡 **현실 팁**: FTP·DNS는 개발 자체에 필요 없지만, Phase 6에서 막히면 며칠 지연됨. Phase 3 시작 즈음에 "곧 보여드릴 수 있으니 Cafe24 FTP 정보 미리 받아두자" 한 마디 던져서 일찍 확보해두는 게 안전.
...
- **Cloudflare Pages** 또는 **Vercel** 무료 티어에 임시 배포
...
**이 문서는 v1.1이며, 개발 진행 중 발견되는 결정 사항은 본 문서에 추가·갱신함.**
